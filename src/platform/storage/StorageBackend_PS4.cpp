// StorageBackend_PS4.cpp — PlatformStorage on the PS4 homebrew filesystem.
//
// The OpenOrbis libc exposes the console's filesystem through ordinary POSIX
// calls, so this is the Wii backend's shape: PosixFileSystem for real paths
// (worlds and options under /data/opticraft, which is writable and survives
// reinstalling the package) and PakStorage for "pak://" paths inside a
// mounted assets.pak, so a world shipped inside it -- the legacy tutorial --
// opens through the same save-format code as one on disk.
