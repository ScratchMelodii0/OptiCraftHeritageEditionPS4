// File_ps4.cpp — PS4 implementation of java/File.h.
//
// The OpenOrbis libc gives a real POSIX filesystem, so this is the Wii version
// (src/wii/java/File_wii.cpp) minus its libfat mount check: paths are used
// verbatim (no realpath(), no $HOME), and the install directory comes from
// GameResources::getExeDir() -- see Ps4Paths.
#ifdef PS4_PLATFORM
...
public:
	...
	{
		// /data is an ordinary writable filesystem, so worlds persist here.
